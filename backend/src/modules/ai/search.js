...
import { searchHotels } from '../inventory/search.js';
...
const TOOL = {
  name: 'search_hotels',
  description: 'Search bookable hotel rooms. Fill only what the traveller stated or clearly implied.',
  parameters: {
    type: 'OBJECT',
    properties: {
      city: { type: 'STRING', description: 'City name in English, exactly as in the allowed list.' },
      check_in_date: { type: 'STRING', description: 'YYYY-MM-DD arrival date.' },
      nights: { type: 'INTEGER', description: 'Number of nights.' },
      rooms: { type: 'INTEGER', description: 'Number of rooms.' },
      adults: { type: 'INTEGER', description: 'Number of adult guests in total.' },
      max_price_per_night: { type: 'NUMBER', description: 'Budget ceiling per night.' },
      currency: { type: 'STRING', description: 'ISO-4217 code of the budget, e.g. INR for ₹ / रुपये.' },
      star_rating: { type: 'INTEGER', description: 'Minimum star rating (1-5).' },
      preferences: {
        type: 'ARRAY',
        items: { type: 'STRING', enum: ['breakfast', 'refundable'] },
        description: 'breakfast = breakfast included / नाश्ता; refundable = free cancellation.',
      },
    },
    required: ['city'],
  },
...
async function parseWithGemini(text, today, names) {
  const system =
    `You turn hotel-search requests, written in English or Hindi, into a search_hotels call. ` +
    `Today is ${today}. Cities must be one of: ${names.join(', ')} — translate Devanagari or other ` +
    `spellings to the English name from that list. If the traveller names a region or an old name ` +
    `that is not in the list, choose the listed city that stands for it (Goa → Panaji, Bombay → Mumbai, ` +
    `Bangalore → Bengaluru, Delhi → New Delhi). Resolve relative or partial dates to the next ` +
    `future occurrence; for a range like "Dec 15–17" nights = end minus start. "under ₹5000" means ` +
    `max_price_per_night 5000 with currency INR. Convert Devanagari digits. Never invent values the ` +
    `traveller did not give.`;
  ...
  try {
    const json = await callGemini({
      contents: [{
        role: 'user',
        parts: [{
          text: `In 2 short sentences, ${lang === 'hi' ? 'in Hindi' : 'in English'}, compare these hotel options for a ` +
            `traveller, noting price / rating trade-offs and any scarce availability. Use only this data: ${JSON.stringify(top)}`,
        }],
      }],
    });
