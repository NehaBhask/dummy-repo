...
| # | ≈ Time | Do | Say | Proves |
|---|---|---|---|---|
| 1 | 0:00 | Click the Hindi chip `जयपुर में 2 रातों के लिए होटल, ₹5000 से कम`. | "Search in your own language. Gemini fills in city, dates and budget; every room you see is a real database row — the model never invents inventory." | AI search, multilingual, grounded |
...
- **Gemini down / slow:** use the chips (cached). If a free-typed query hangs, say "that's the live model; the safety net is the cache" and go back to a chip, or use the structured search form — no AI dependency.
...
- **Venue wifi drops:** steps 1–6 run entirely on the laptop (AI chips are cached). Only step 7 needs the network — hence pre-loaded.
