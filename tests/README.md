...
npm test                             # node --test over ../tests/*.test.js — 48 tests, ~13 s
```

Baseline: **48 tests · 45 pass · 3 skip · 0 fail.** The 3 skips are the offline-fallback AI tests, which skip
...
| `ai.test.js` | 6 | Heuristic parser (demo query, aliases, no invented values), AI results are grounded in real rows and logged, Hindi without a key is a clear `ai_unavailable`, unknown/no-inventory cities ask or explain rather than guess. |
